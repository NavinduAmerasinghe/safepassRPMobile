...
TaskManager.defineTask(LOCATION_TRACKING, async ({ data, error }) => {
    ...
    if (data) {
        ...
        const result = await fetch("http://192.168.1.70:8000/api/observationsforLocation",
            {
                method: "POST",
                headers: {
            ...
            if(resultData.data.newObservation.length>0){
              for (let index = 0; index < resultData.data.newObservation.length; index++) {
                console.log(resultData.status);
                if(resultData.status==="Warning"){
                  schedulePushNotification(resultData.data.newObservation[index].animalName);
              }
...
function UserLocation() {
    ...
    React.useEffect(() => {
        const config = async () => {
            let loc_back = await Permissions.askAsync(Permissions.LOCATION_BACKGROUND)
